...
class PickAndPlaceDemo:
    def __init__(self):
        ...
        # 设置目标位置所使用的参考坐标系
        #arm.set_pose_reference_frame('ar_marker_0')
        ...
        while not rospy.is_shutdown():
            try:
                (trans,rot) = listener.lookupTransform('ar_marker_0', 'ee_link', rospy.Time(0))
                break
            except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
        ...
        grasp_pose.pose.position.z =  0.13
    ...
    def make_grasps(self, initial_pose_stamped, allowed_touch_objects):
        ...
        # 欧拉角到四元数的转换             
        # 设置抓取的姿态
        g.grasp_pose.pose.orientation.x = -0.5
        g.grasp_pose.pose.orientation.y = 0.5
        g.grasp_pose.pose.orientation.z = 0.5
        g.grasp_pose.pose.orientation.w = 0.5
    ...
    def make_places(self, init_pose):
        ...
        for y in y_vals:
            for x in x_vals:
                ...
                # 欧拉角到四元数的转换
                #q = quaternion_from_euler(-0.298, 1.570, 1.273)
                
                # 欧拉角到四元数的转换
                place.pose.orientation.x = -0.5
                place.pose.orientation.y = 0.5
                place.pose.orientation.z = 0.5
                place.pose.orientation.w = 0.5
