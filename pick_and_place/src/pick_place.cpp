...
void pick(moveit::planning_interface::MoveGroupInterface& move_group,const geometry_msgs::Pose &marker_pose)
{
  ...
  if(marker_pose.position.z>0.05){
...
void addCollisionObjects(moveit::planning_interface::PlanningSceneInterface& planning_scene_interface,const geometry_msgs::Pose &marker_pose)
{
  ...
  collision_objects[2].primitives[0].dimensions[2] = marker_pose.position.z+0.05;
  ...
  collision_objects[2].primitive_poses[0].position.z = (marker_pose.position.z+0.05)/2.0-0.05;
...
int main(int argc, char** argv)
{
  ...
  pick(group,marker_pose);

  ros::WallDuration(1.0).sleep();

  place(group,marker_pose);
